...
def gen_inequalities(possible_transitions):
    # Vertex representation of the possible transitions of the BCT
...
if __name__ == "__main__":
    ...
    for val in data:
        ...
        possible_transitions, impossible_transitions = get_transitions(input_bit_size, output_bit_size, BCT)
        ...
        f2 = open(f"BCT_Greedy_4-bit_sboxes.txt","a")
        f2.write(name + " - " + str(len(final_ineqs_first)) + "\n")
        f2.close()

        f1 = open(f"4-bit_sboxes/{name}_Greedy_BCT.txt","a")
        for q in final_ineqs_first:
            f1.write(str(list(q))+"\n")
        f1.close()
